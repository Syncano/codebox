		...
		Convey("given mocked Upload stream, Upload", func() {
			...
			Convey("given proper meta and chunk data", func() {
				...
				Convey("stores file in repo", func() {
					...
					repo.On("Store", "someKey", "storeKey", mock.Anything, "someName", os.ModeCharDevice).Return("path", nil)
				...
				Convey("stores multi-part file in repo", func() {
					...
					ch := make(chan struct{})
					repo.On("StoreLock", "someKey").Return(ch, "storeKey").Once()
					repo.On("Store", "someKey", "storeKey", mock.Anything, "someName", os.ModeCharDevice).Return("path", nil).Run(func(args mock.Arguments) {
						buf := make([]byte, 16)
				...
				Convey("propagates multi-file store error", func() {
					...
					ch := make(chan struct{})
					repo.On("StoreLock", "someKey").Return(ch, "storeKey").Once()
					repo.On("Store", "someKey", "storeKey", mock.Anything, "someName", os.ModeCharDevice).Return("path", err)
