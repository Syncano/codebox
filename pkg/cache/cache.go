...
// See LRUCache and StackCache for example implementation.
type Cache struct {
	options Options
	len     int
...
type DeleteHandler func(*valuesItem, interface{}) *keyValue
...
func (c *Cache) defaultDeleteHandler(item *valuesItem, val interface{}) *keyValue {
	delete(c.valueMap, item.key)
	return &keyValue{key: item.key, value: val.(*Item).object}
}
...
func (c *Cache) deleteValue(e *list.Element, now int64) (valueEvicted *keyValue) {
	...
	if val, ok := c.valueMap[item.key]; ok {
		valueEvicted = c.deleteHandler(item, val)
	}
	c.len--
